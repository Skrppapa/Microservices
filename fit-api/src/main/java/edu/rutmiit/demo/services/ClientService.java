...
public class ClientService {
    ...
    public ClientService(InMemoryStorage storage, @Lazy GymService gymService) {
    ...
    public PagedResponse<ClientResponse> findAll(String firstName, String lastName, String city, int page, int size) {
        List<ClientResponse> all = storage.clients.values().stream()
                .filter(c -> firstName == null || c.firstName().toLowerCase().contains(firstName.toLowerCase()))
                .filter(c -> lastName == null || c.lastName().toLowerCase().contains(lastName.toLowerCase()))
                .filter(c -> city == null || (c.city() != null && c.city().toLowerCase().contains(city.toLowerCase())))
    ...
    public ClientResponse update(Long id, ClientRequest request) {
        ...
        ClientResponse updatedClient = new ClientResponse(id,
                request.firstName() == null ? existing.firstName() : request.firstName(),
                request.lastName() == null ? existing.lastName() : request.lastName(),
                request.birthDate() == null ? existing.birthDate() : request.birthDate(),
