...
@Tag(name = "memberships", description = "API для работы с абонементами")
...
public interface MembershipApi {
    ...
    @GetMapping("/{id}")
    MembershipResponse getMembershipById(@PathVariable("id") Long id);
    ...
    @GetMapping
    PagedResponse<MembershipResponse> getAllMemberships(
    ...
    MembershipResponse createMembership(@Valid @RequestBody MembershipRequest request);
    ...
    @ApiResponse(responseCode = "409", description = "Абонемент с таким номером уже существует", content = @Content(schema = @Schema(implementation = StatusResponse.class)))
    @PutMapping("/{id}")
    MembershipResponse updateMembership(@PathVariable Long id, @Valid @RequestBody MembershipRequest request);
